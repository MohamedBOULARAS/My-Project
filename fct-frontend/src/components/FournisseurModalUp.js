...
export default function FournisseurModal({setFournisseur,fournisseur,allFournisseurs}) {
  ...
  //hook modal
  const [codeFournisseur, setCodeFournisseur] = useState(fournisseur.code_fournisseur);
  const [raisonSocial, setRaisonSocial] = useState(fournisseur.raison_social);
  const [nomFournisseur, setNomFournisseur] = useState(fournisseur.nom);
  const [prenomFournisseur, setPrenomFournisseur] = useState(fournisseur.prenom);
  const [emailFournisseur, setEmailFournisseur] = useState(fournisseur.email);
  const [tellFournisseur, setTellFournisseur] = useState(fournisseur.tell);
  const [adresseFournisseur, setAdresseFournisseur] = useState(fournisseur.adresse);
  const [activiteFournisseur, setActiviteFournisseur] = useState(fournisseur.activite);
  const [nifFournisseur, setNifFournisseur] = useState(fournisseur.nif);
  const [nisFournisseur, setNisFournisseur] = useState(fournisseur.nis);
  const [ribFournisseur, setRibFournisseur] = useState(fournisseur.rib);
  const [rcFournisseur, setRcFournisseur] = useState(fournisseur.rc);
  const [noteFournisseur, setNoteFournisseur] = useState(fournisseur.note);
    ...
    const addFournisseur = (e) => {
      ...
      //fetch Post Data
      fetch("http://localhost:5000/fournisseur/"+fournisseur._id, requestOptions)
      .then(response => response.json())
      .then((res) => {
        console.log(res)
        var temp = allFournisseurs
        temp[temp.indexOf(fournisseur)] = newFournisseur
        setFournisseur([...temp])      
    })
      setOpen(false);
    }
