...
import ClientModalUp from '../ClientModalUp.js';
...
const print = () => {
  ...
  // Table
  const clientCol = ["Code", "Client", "e-mail", "Tel", "Adresse"];

  const clientRows =  client.map(c => {
    const row = [c.code_client, c.raison_social, c.email, c.tell, c.adresse];
    return row;
  });

  // const startY = 10 * 2.83;
  const startY = 30 * 2.83;
  doc.autoTable(clientCol, clientRows, {
    // startY: 180 * 2.83,
    startY,
    theme: "striped",
    styles: {
  ...
  doc.save("Liste des clients.pdf");
};
        ...
        <div className='client-cntt'>
          ...
          <div className='div-recherche-client'>
            ...
            <div className='client-btns'>
              <button className='client-btn-imprimer' onClick={print}>Imprimer</button>
            </div>
          </div>

          <div className='client-excel'>
                ...
                <TableBody>
                  ...
                  {client.filter(item => { //recherche
                    if (recherche == "") {
                     ...
                     || item.code_client?.toString().toLowerCase().includes(recherche.toLowerCase())
                    ...
                    .map((item) => { //création de tableau
                      return (
                        <StyledTableRow >
                          ...
                          <ClientModalUp allClients={client} client={client.find(el => el._id == item._id)} setClient={setClient} />
                            <Button  
                            onClick={ () => {
                             setConfirmDialog({
                                isOpen: true,
