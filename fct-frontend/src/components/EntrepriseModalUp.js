...
export default function EntrepriseModal({setEntreprise,entreprise,allEntreprises}) {
  ...
  //hook modal
  const [codeEntreprise, setCodeEntreprise] = useState(entreprise.code_entreprise);
  const [raisonSocial, setRaisonSocial] = useState(entreprise.raison_social);
  const [typeEntreprise, setTypeEntreprise] = useState(entreprise.type_entreprise);
  const [emailEntreprise, setEmailEntreprise] = useState(entreprise.email);
  const [tellEntreprise, setTellEntreprise] = useState(entreprise.tell);
  const [adresseEntreprise, setAdresseEntreprise] = useState(entreprise.adresse);
  const [activiteEntreprise, setActiviteEntreprise] = useState(entreprise.activite);
    ...
    const addEntreprise = (e) => {
      ...
      //fetch Post Data
      fetch("http://localhost:5000/entreprise/"+entreprise._id, requestOptions)
      .then(response => response.json())
      .then((res) => {
        console.log(res)
        var temp = allEntreprises
        temp[temp.indexOf(entreprise)] = newEntreprise
        setEntreprise([...temp])
      })
      setOpen(false);
    }
  ...
  return (
    <div>
      <div>
      <Button  ><i class="fas fa-pen" onClick={handleOpen}></i></Button>
      </div>
      <Modal
        open={open}
        onClose={handleClose}
