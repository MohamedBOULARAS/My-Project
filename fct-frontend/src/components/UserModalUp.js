...
export default function UserModal({setUser,user,allUsers}) {
  ...
  //hook modal
  const [codeUser, setCodeUSer] = useState(user.code_user);
  ...
  const [selectedEntreprise, setSelectedEntreprise] = useState(0);
    ...
    const addUser = (e) => {
      ...
      var newUser = {code_user: codeUser, username: userName, firstname: firstName, lastname: lastName, email: userEmail,  entr:[{ raison_social : entreprise[selectedEntreprise].raison_social }]}
      console.log({selectedEntreprise}, {newUser})
      ...
      //fetch Post Data
      fetch("http://localhost:5000/register/"+user._id, requestOptions)
      .then(response => response.json())
      .then((res) => {
        console.log(res)
        var temp = allUsers
        temp[temp.indexOf(user)] = newUser
        setUser([...temp])
      })
      setOpen(false);
